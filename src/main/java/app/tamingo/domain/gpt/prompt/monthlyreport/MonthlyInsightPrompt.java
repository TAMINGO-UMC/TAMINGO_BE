package app.tamingo.domain.gpt.prompt.monthlyreport;
...
@Component
public class MonthlyInsightPrompt implements PromptTemplate {

    private final String allowedTypes = Arrays.stream(MonthlyInsightType.values())
            .map(Enum::name)
    ...
    public String render() {
        return String.join("\n",
                "[ROLE]",
                "너는 사용자의 월간 기록을 분석해 월간리포트 인사이트 3개를 생성하는 도우미다.",
                "",
                "[INPUT]",
                "- MONTHLY_REPORT_DATA에 포함된 데이터만 근거로 사용한다.",
